...
import { User, Phone, Calendar, DollarSign, Shield, Trash2, CheckCircle2, AlertCircle, ChevronDown, ChevronUp, CreditCard, Pencil } from 'lucide-react';
...
const TenantCard = ({ tenant, onMarkPaid, onDelete, onEdit }: TenantCardProps) => {
      ...
      <CardContent className="p-4 space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${isPaid ? 'bg-[hsl(var(--tenant-paid-border))]' : 'bg-tenant-due-border'}`}>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>KSh {tenant.monthlyRent.toLocaleString()}/mo</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Shield className="w-3.5 h-3.5 text-muted-foreground" />
            <span>KSh {tenant.securityDeposit.toLocaleString()}</span>
          </div>
          <div className="flex items-center gap-1.5 col-span-2">
        ...
        {hasCarryForward && (
          <div className="text-xs bg-primary/10 text-primary rounded-md px-2 py-1">
            💰 KSh {currentMonthStatus.carryIn.toLocaleString()} credit from overpayment — only KSh {Math.max(0, tenant.monthlyRent - currentMonthStatus.carryIn).toLocaleString()} due this month
          </div>
        )}

        <div className="flex items-center justify-between pt-1 border-t border-border/50">
          {isPaid ? (
            <Badge variant="secondary" className="bg-[hsl(var(--tenant-paid-border))] text-tenant-paid-text text-xs gap-1">
              <CheckCircle2 className="w-3 h-3" /> Fully Paid
            </Badge>
          ) : (
            <Badge variant="secondary" className="bg-tenant-due-border text-tenant-due-text text-xs gap-1">
              <AlertCircle className="w-3 h-3" /> Arrears: KSh {totalDue.toLocaleString()}
            </Badge>
          )}
